...
public Action:SlowCube_Timer(Handle:timer, Handle:dataPackHandle)
{
	...
	for (new i = 1; i <= MaxClients ; i++)
	{
		...
		if(client_rolls[i][AWARD_G_SLOWCUBE][3] == 0)
		{
			...
			if(((GetRandomInt(0,100) > (100 - RTD_Perks[i][10])) && GetTime() >= client_rolls[i][AWARD_G_SLOWCUBE][7]) || GetTime() <= client_rolls[i][AWARD_G_SLOWCUBE][6])
			{
				if(GetTime() > client_rolls[i][AWARD_G_SLOWCUBE][6])
				{
					//next time when bypassing is allowed (10s)
					client_rolls[i][AWARD_G_SLOWCUBE][7] = GetTime() + 10;
					
					//immunity time
					client_rolls[i][AWARD_G_SLOWCUBE][6] = GetTime() + 2;
					
					//PrintToChat(i, "SlowCube Immunity starting: %i", GetTime());
				}
