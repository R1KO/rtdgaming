...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_bmdebug", Command_BMDebug, ADMFLAG_GENERIC, "Toggle Debugging messages sent to you(spammy)");
...
public Action:Timer_Debug(Handle:timer,any: client)
{
	if(!bm_options[client][2])
		KillTimer(timer);
	Format(message, sizeof(message), "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 Generic p# %d - b# %d - c# %d - s# %d - bc# %d - ba# %d", playerCount, botCount, clientCount, spectatorCount, botCmdCount, botAutoCount);
	PrintToChatSome(message, 3);
	return Plugin_Continue;
}
...
public Action:Timer_CheckBots(Handle:timer)
{
	...
	if(cmd_bot_kick)
	{
		...
		{
			Format(message, sizeof(message), "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 CmdBotKick CheckTotal %d", (botCount+playerCount+spectatorCount));
			PrintToChatSome(message, 3);
	...
	if(botsVsHumans)
	{
		...
		if((botCount+playerCount+spectatorCount+1) >= MaxClients && ((botCount-botCmdCount) == maxbVhBots || maxbVhBots >= MaxClients))
		{
			...
			Format(message, sizeof(message), "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 bVhbots >=+1 maxclient CheckTotal %d - maxbvh %d", (botCount+playerCount+spectatorCount+1), maxbVhBots);
			PrintToChatSome(message, 3);
		}
		if(maxbVhBots < 0)
			maxbVhBots = 0;
		if((botCount-botCmdCount) < maxbVhBots)
		{
			...
			Format(message, sizeof(message), "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 bVhbots > max bVhBots %d - maxbvh %d", (botCount-botCmdCount), maxbVhBots);
			PrintToChatSome(message, 3);
		}
	...
	else
	{
		...
		{
			Format(message, sizeof(message), "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 Incorrect Auto Count CheckTotal %d - maxAuto %d - b# %d - bA# %d bC %d", (botAutoCount+botCmdCount), maxAutoBots, botCount, botAutoCount, botCmdCount);
			PrintToChatSome(message, 3);
		...
		if((botAutoCount+playerCount+spectatorCount+1) >= MaxClients && maxAutoBots == botAutoCount)
		{
			...
			Format(message, sizeof(message), "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 autobots =>+1 maxclient CheckTotal %d - maxauto %d", (botAutoCount+playerCount+spectatorCount+1), maxAutoBots);
			PrintToChatSome(message, 3);
		}
		...
		if(botAutoCount < maxAutoBots)
		{
			...
			Format(message, sizeof(message), "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 autobots < max AutoCount %d - maxAuto %d", botAutoCount, maxAutoBots);
			PrintToChatSome(message, 3);
		}
...
public Action:Command_BMDebug(client, args)
{
	new String:arg[128];
	GetCmdArg(1, arg, sizeof(arg));
	if(StrEqual(arg, "reload", false))
	{
		PrintToChat(client, "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 Reloading BotManager.");
		ServerCommand("sm plugins reload botmanager");
	}
	else if(StrEqual(arg, "unload", false))
	{
		KickBots(4);
		PrintToChat(client, "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 Unloading BotManager.");
		ServerCommand("sm plugins unload botmanager");
	}
	else if(StrEqual(arg, "events1", false))
		ServerCommand("sm_bm_supressevents 1");
	else if(StrEqual(arg, "events0", false))
		ServerCommand("sm_bm_supressevents 0");
	else if(StrEqual(arg, "info", false))
	{
		bm_options[client][2] = 1;
		PrintToChat(client, "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 Enabling Debug Timer. Disable Debug to Kill the Timer.");
		CreateTimer(5.0,Timer_Debug,client,TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
	}
	else if(StrEqual(arg, "auto++", false))
	{
		botAutoCount++;
		PrintToChat(client, "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 New botAutoCount %d.", botAutoCount);
	}
	else
	{
		if(bm_options[client][2])
		{
			bm_options[client][2] = 0;
			PrintToChat(client, "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 Disabled.");
		}
		else
		{
			bm_options[client][2] = 1;
			PrintToChat(client, "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 Enabled.");
		}
	}
...
UpdateBotCounts(method=1)
{
	...
	{
		botAutoCount--;
		Format(message, sizeof(message), "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 Update Auto");
		PrintToChatSome(message, 3);
	}
	else if(method == 3)
	{
		//bVh Kick should not ever update counts
		Format(message, sizeof(message), "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 Update bVh");
		PrintToChatSome(message, 3);
	}
	else
	{
		Format(message, sizeof(message), "\x04[\x03BotMan\x04][\x03Debug\x04]\x01 Update Normal");
		PrintToChatSome(message, 3);
		if(botCmdCount >= 1)
		{
			botCmdCount--;
...
DisplayAdminBotManagerMenu(client)
{
	...
	AddMenuItem(menu,("Option %i",client),	menuDescription2);
...
PrintToChatSome(const String:msg[], type=1 )
{
	...
	for(new i=1; i <= MaxClients; i++)
	{
		...
		if(IsClientInGame(i) && bm_options[i][0]  && type == 2)
			continue;
		if(IsClientInGame(i) && !bm_options[i][2] && type == 3)
			continue;
		if(IsClientInGame(i))
			PrintToChat(i,msg);
	}
